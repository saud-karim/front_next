...
export default function ProductsPage() {
  ...
  const [apiProducts, setApiProducts] = useState<MultilingualProduct[]>([]);
  ...
  const handleAddToCart = (product: MultilingualProduct) => {
    addToCart({
      id: product.id,
  ...
  const handleWishlistToggle = (product: MultilingualProduct) => {
    if (!isLoggedIn) {
  ...
  // Use only apiProducts to avoid type conflicts
  ...
  if (searchTerm) {
    filteredProducts = filteredProducts.filter(product => {
      ...
      const searchInFeatures = product.features.some(feature => {
        const featureText = typeof feature === 'string' ? feature : getLocalizedText(feature, language);
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    switch (sortBy) {
      case 'price-low':
        const priceA = typeof a.price === 'string' ? parseInt(a.price.slice(1).replace(',', '')) : a.price;
        const priceB = typeof b.price === 'string' ? parseInt(b.price.slice(1).replace(',', '')) : b.price;
        return priceA - priceB;
      case 'price-high':
        const priceHighA = typeof a.price === 'string' ? parseInt(a.price.slice(1).replace(',', '')) : a.price;
        const priceHighB = typeof b.price === 'string' ? parseInt(b.price.slice(1).replace(',', '')) : b.price;
      ...
      default:
        const nameA = typeof a.name === 'string' ? a.name : getLocalizedText(a.name, language);
        const nameB = typeof b.name === 'string' ? b.name : getLocalizedText(b.name, language);
                ...
                >
                  ...
                  <div className="relative p-6 bg-gradient-to-br from-gray-50 to-gray-100 text-center">
                    <div className={`absolute top-3 left-3 ${product.badgeColor} text-white text-xs font-bold px-2 py-1 rounded-full`}>
                      {typeof product.badge === 'string' ? product.badge : getLocalizedText(product.badge, language)}
                    </div>
